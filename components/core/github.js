...
async function huggingfaceRequest(repoType, ownerName, repoName, url, authorize = true, buffer = false) {
	if(typeof SettingsManager != 'undefined' && window.api)
		window.api.hf_token = SettingsManager.get('core', 'huggingfaceToken');

	// repoType should be 'models' or 'datasets' based on HF API spec
	const type = repoType === 'datasets' ? 'datasets' : 'models';
	const repoId = `${ownerName}/${repoName}`;

	// Construct Hugging Face endpoint routing url
	const baseUrl = `https://huggingface.co/api/${type}/${repoId}`;
	...
	try {
		...
		// Apply Bearer authorization if requested and available
		if(authorize && token) {
			headers['Authorization'] = `Bearer ${token}`;
		} else if(!token) {
			console.warn('No Hugging Face Token provided. Access limited to public repositories.');
		}
		...
		// Fallback: try unauthenticated request if authorized request failed due to permissions
		if(authorize && up.message === 'UNAUTHORIZED_ACCESS') {
			return await huggingfaceRequest(repoType, ownerName, repoName, url, false, buffer);
		}
		console.error("Failed to huggingface: " + fullUrl, up);
		throw up;
	}
}
...
async function getHuggingFaceBranch(repoType, owner, repo) {
	const repoKey = `${repoType}/${owner}/${repo}`;
	if(defaultBranches[repoKey]) return defaultBranches[repoKey];

	// Requesting the root repository URL gives us the core repo metadata
	const data = await huggingfaceRequest(repoType, owner, repo, '');

	// Hugging Face uses 'defaultBranch' (camelCase) instead of 'default_branch'
	const defaultBranch = data.defaultBranch || 'main';
	defaultBranches[repoKey] = defaultBranch;
	return defaultBranch;
}



async function getHuggingFaceVersion(repoType, owner, repo, branch) {
	// Fallback to default branch if not specified
	branch ||= await getHuggingFaceBranch(repoType, owner, repo);

	// 1. Query the specific revision/branch endpoint
	const branchData = await huggingfaceRequest(repoType, owner, repo, `/revision/${encodeURIComponent(branch)}`);

	// 2. Extract the date string directly from the root of the revision payload
	const dateString = branchData.commitDate;
